...
root.hcl    Single source of truth for backend + provider — a `remote_state` block (its own
            ...
            (`use_lockfile = true`) for every layer including `state` itself — no DynamoDB table
            anywhere in this module, see kit-modules/basis/CLAUDE.md.
envs/
...
- Full details (dependency-block mechanics, the provider-module output contract, state locking):
  see `kit-modules/basis/CLAUDE.md`, which is auto-loaded on demand when reading files under that
