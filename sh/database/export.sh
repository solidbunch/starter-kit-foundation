...
CONTAINER_OUTPUT_FILE="/tmp/$DATABASE_CONTAINER-$MYSQL_DATABASE-$ENVIRONMENT_TYPE-$APP_DOMAIN-$(date +%Y-%m-%d).sql"

echo "Exporting local database to '${OUTPUT_FILE}'. It can take more than a few minutes. Please wait."

# Export data to sql file
docker compose exec "${DATABASE_CONTAINER}" bash -c "mariadb-dump -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DATABASE} > ${CONTAINER_OUTPUT_FILE}"
